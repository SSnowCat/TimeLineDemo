    ...
    */
    ...
    override func draw(_ rect: CGRect) {
        ...
        for i in 0..<modelArray.count {
            let model:TimeLineModel = modelArray[i]
            ...
            if i%2 == 0  {
                ...
                if !model.address.isEmpty {
                    ...
                    addressLabel.text = model.address + "\(i)"
                    addressLabel.sizeToFit()
                    self.addSubview(addressLabel)
                    
                }
                //目的
                if !model.address.isEmpty && !model.motive.isEmpty {
                    ...
                    motiveLabel.text = model.motive + "\(i)"
                    motiveLabel.sizeToFit()
                    self.addSubview(motiveLabel)
                    
                }else if !model.address.isEmpty {
                    let motiveImage = UIImageView(frame: CGRect(x: endPoint.x-20.0, y: endPoint.y+40, width: 15, height: 15))
                    motiveImage.image = UIImage(named: "message.png")
                    self.addSubview(motiveImage)
                    let motiveLabel = UILabel(frame: CGRect(x: motiveImage.frame.maxX + 5, y: endPoint.y+40, width: CGFloat(WIDTH/2), height: 20.0))
                ...
                //时间
                if model.address.isEmpty && model.motive.isEmpty && !model.date.isEmpty {
                    ...
                    dateLabel.text = model.date + "\(i)"
                    dateLabel.sizeToFit()
                    self.addSubview(dateLabel)
                    
                }else if model.address.isEmpty && !model.motive.isEmpty && !model.date.isEmpty {
            ...
            ctf?.setFillColor(model.color.cgColor)
            ctf?.drawPath(using: CGPathDrawingMode.eoFillStroke)
            //绘制中线上的原点
